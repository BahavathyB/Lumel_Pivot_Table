...
const PivotTable: React.FC = () => {
  ...
  // -------------------- PAGINATED FLAT ROW DATA (using useEffect) --------------------
  useEffect(() => {
    const calculatePaginatedFlatRowData = () => {
      const startIndex = page * rowsPerPage;
      const endIndex = startIndex + rowsPerPage;
      const pageNonSubtotalRows = nonSubtotalRows.slice(startIndex, endIndex);

      if (pageNonSubtotalRows.length === 0) {
        setPaginatedFlatRowData([]);
        return;
      }
      ...
      for (let i = lastIndex + 1; i < flatRowData.length; i++) {
        ...
        if (row?.isSubtotal) {
          const subtotalParentPath = row.keys
            .slice(0, (row.subtotalLevel ?? 0) + 1)
            .join("|");
          const belongsToPage = pageNonSubtotalRows.some((pageRow) => {
            const pageRowPath = pageRow.keys
              .slice(0, (row.subtotalLevel ?? 0) + 1)
              .join("|");
      ...
      setPaginatedFlatRowData(
        flatRowData.slice(firstIndex, endIndexInclusive + 1)
      );
    };

    calculatePaginatedFlatRowData();
  ...
  // -------------------- PIVOT DATA CALCULATION (using useEffect) --------------------
  useEffect(() => {
        ...
        if (columns.length > 0) {
          leafColumnKeys.forEach((colKey) => {
            ...
            if (!rowData.isSubtotal) {
              ...
              if (matchingRows.length > 0) {
                data[rowData.key]![colKey] =
                  calculateAggregations(matchingRows);
              }
            }
          });
        } else {
